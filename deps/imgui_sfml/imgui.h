...
namespace ImGui
{
    ...
    IMGUI_API void          SetStateStorage(ImGuiStorage* storage);                             // replace current window storage with our own (if you want to manipulate it yourself, typically reset subsection of it)
    ...
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and reset io.WantSaveIniSettings.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we reset data so that IsItemHovered() will return false, to avoid subsequent user code submitting tooltips. This flag disable this behavior so you can still call IsItemHovered() on the source item.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoMouse                = 1 << 4,   // Instruct imgui to reset mouse position/buttons in NewFrame(). This allows ignoring the mouse information set by the back-end.
...
// - Important: reset() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
...
struct ImGuiIO
{
    ...
    bool        WantSaveIniSettings;            // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: reset io.WantSaveIniSettings yourself after saving!
...
struct ImDrawListSplitter
{
    ...
    inline void                 Clear() { _Current = 0; _Count = 1; } // Do not reset Channels[] so our allocations are reused next frame
